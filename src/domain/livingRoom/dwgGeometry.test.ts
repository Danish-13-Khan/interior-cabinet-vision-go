...
describe('DWG preview', () => {
  it('uses geometry bounds and inverts CAD Y without changing dimensions', () => {
    ...
    expect(preview.layers[0].paths[0]).toBe('M10,-20 L610,-320');
  });
  it('reports blocks, unsupported curves, invalid and parser-omitted entities', () => {
    ...
    expect(preview.omitted).toEqual({ INSERT: 1, 'LWPOLYLINE (curved/elevated)': 1, 'LINE (invalid)': 1, 'Parser unsupported entities': 2 });
  });
  ...
  it('handles closed polylines and circle extents', () => {
    const preview = buildDwgPreview(drawing([{ type: 'LWPOLYLINE', layer: '0', flag: 1, vertices: [{ x: 0, y: 0 }, { x: 5, y: 5 }] }, { type: 'CIRCLE', center: { x: 0, y: 0 }, radius: 10 }]));
    expect(preview.bounds).toEqual({ minX: -10, minY: -10, maxX: 10, maxY: 10 });
    expect(preview.layers[0].paths[0]).toContain(' Z');
  });
  it('rejects drawings with no supported area', () => {
    expect(() => buildDwgPreview(drawing([{ type: 'INSERT' }]))).toThrow(/INSERT: 1/);
  });
