...
describe("planUnderlayAabb", () => {
  it("is the unrotated rectangle around the underlay centre", () => {
    expect(planUnderlayAabb(sample())).toEqual({
      minX: -6000,
      maxX: 6000,
      minZ: -4500,
      maxZ: 4500,
  ...
  it("grows when the plan is rotated 45 degrees", () => {
    const box = planUnderlayAabb(sample({ rotationDeg: 45 }));
    expect(box.minX).toBeLessThan(-6000);
