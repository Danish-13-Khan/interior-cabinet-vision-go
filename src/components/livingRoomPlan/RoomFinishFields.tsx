...
function FinishUv({
  project, materialId, onSetFinishUv,
}: { project: InteriorProject; materialId: string | null; onSetFinishUv: Props["onSetFinishUv"] }) {
  ...
  return <>
    <NumberField label="Tile mm" value={Number(finish.extensions?.uvScaleMm) || 1000}
      onChange={(uvScaleMm) => onSetFinishUv(finish.id, { uvScaleMm })} />
    <NumberField label="Rotate °" value={Number(finish.extensions?.uvRotationDeg) || 0}
      onChange={(uvRotationDeg) => onSetFinishUv(finish.id, { uvRotationDeg })} />
  </>;
}
...
export function RoomFinishFields(props: Props) {
  ...
  return (
    <div className="lr-room-finishes">
      ...
      <FinishUv project={props.project} materialId={floorId} onSetFinishUv={props.onSetFinishUv} />
      ...
      <FinishUv project={props.project} materialId={ceilingId} onSetFinishUv={props.onSetFinishUv} />
    </div>
  );
}
