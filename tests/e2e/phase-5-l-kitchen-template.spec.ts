...
test("Phase 5 L Kitchen: template → L run + appliances → 3D → save → reopen", async ({ page }) => {
  ...
  await page.getByTestId("catalog-template-template:core:l-kitchen:v1").click();
  ...
  expect(download.suggestedFilename().toLowerCase()).toMatch(/kitchen|cabinet|project|\.json$/);

  await page.getByTestId("interiors-project-crumb").evaluate((button: HTMLButtonElement) => button.click());
  const recent = page.getByTestId("open-recent-project").filter({ hasText: "L Kitchen" }).first();
