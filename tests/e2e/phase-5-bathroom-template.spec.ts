...
test("Phase 5 Bathroom: template → 2D → finish → 3D → save → reopen", async ({ page }) => {
  ...
  await page.getByTestId("catalog-template-template:core:bathroom:v1").click();
  ...
  await ceramic.getByTitle("Apply Terracotta Planter").click();
  await expect(ceramic.locator('[data-material-id="material:core:planter-terracotta:v1"]')).toHaveClass(/is-active/);
  ...
  expect(download.suggestedFilename().toLowerCase()).toMatch(/bathroom|cabinet|project|\.json$/);

  await page.getByTestId("interiors-project-crumb").evaluate((button: HTMLButtonElement) => button.click());
  const recent = page.getByTestId("open-recent-project").filter({ hasText: "Bathroom" }).first();
